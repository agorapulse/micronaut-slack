...
requires having `slack.bucket` property set and an AWS SDK v2 `S3Client` bean must be available. The easiest solution
is to add https://agorapulse.github.io/micronaut-aws-sdk/#_simple_storage_service_s3[Micronaut AWS SDK Simple Storage
Service for AWS SDK v2] on the classpath. You also need to have the AWS credentials set up correctly and the bucket
must already exist.
...
----
dependencies {
    implementation 'com.agorapulse:micronaut-amazon-awssdk-s3:5.0.0.RC2'
}
----
